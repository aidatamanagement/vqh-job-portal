  ...
  const {
    ...
    queryFn: async (): Promise<JobApplication[]> => {
      ...
      const transformedData = data?.map((item: any) => ({
        ...
        hrManagerName: item.hr_manager_name,
        hrManagerEmail: item.hr_manager_email,
        isReferredByEmployee: item.is_referred_by_employee,
        referredByEmployeeName: item.referred_by_employee_name,
        hasPreviouslyWorkedAtViaQuest: item.has_previously_worked_at_viaquest,
        lastDayOfEmployment: item.last_day_of_employment,
        certificationSignature: item.certification_signature,
        optInToSMS: item.opt_in_to_sms,
        privacyPolicyAccepted: item.privacy_policy_accepted,
        status: item.status,
        notes: item.notes,
